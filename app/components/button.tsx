...
export const Button = React.forwardRef(function Button(
  ...
  return "href" in props ? (
    ...
    <HeadlessButton
      {...props}
      className={clsx(classes, "cursor-default font-black")}
      ref={ref}
    >
      <TouchTarget>{children}</TouchTarget>
    </HeadlessButton>
...
export function TouchTarget({ children }: { children: React.ReactNode }) {
  return (
    <>
      {children}
      <span
        className="absolute font-black left-1/2 top-1/2 size-[max(100%,2.75rem)] -translate-x-1/2 -translate-y-1/2 [@media(pointer:fine)]:hidden"
        aria-hidden="true"
      />
    </>
  );
